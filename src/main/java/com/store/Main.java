...
public class Main {
    ...
    private static void modifyObject(Scanner scanner, Store store) {
        ...
        try {
            ...
            store.updateOrThrow(existingObject, updatedObject);
            System.out.println("Об'єкт успішно модифіковано.");
        } catch (ObjectNotFoundException exception) {
            System.out.println(exception.getMessage());
        } catch (InvalidFieldValueException exception) {
            System.out.println("Некоректне значення: " + exception.getMessage());
        } catch (IllegalArgumentException exception) {
    ...
    /**
     * Видаляє вибраний елемент одягу з магазину.
     *
     * @param scanner об'єкт для зчитування введення
     * @param store магазин з товарами
     */
    private static void deleteObject(Scanner scanner, Store store) {
        ...
        printClothes(store);
        System.out.print("Введіть номер об'єкта для видалення: ");
        int index = readObjectIndex(scanner, store.getTotalUniqueItems());
        Clothes existingObject = store.getClothes().get(index);

        System.out.print("Підтвердити видалення (так/ні): ");
        if (!readBooleanAnswer(scanner)) {
            System.out.println("Видалення скасовано.");
            return;
        }

        try {
            store.deleteOrThrow(existingObject);
